...
public interface LocalizacionMapper {
    ...
    @Mapping(target = "departamento",
            expression = "java(entity.getDepartamento() != null ? entity.getDepartamento().getNombre() : null)")
    @Mapping(target = "distrito",
            expression = "java(entity.getMunicipio() != null ? entity.getMunicipio().getNombre() : null)")
    ...
    @Mapping(target = "proyecto",
            expression = "java(idProyecto != null ? Proyecto.builder().id(idProyecto).build() : null)")
    // Lo gestiona el servicio buscando/creando en catálogo
    @Mapping(target = "departamento", ignore = true)
    // Lo gestiona el servicio buscando/creando en catálogo
    @Mapping(target = "municipio", ignore = true)
    @Mapping(target = "direccion", source = "filaDto.direccionEspecifica")
    @Mapping(target = "latitud", expression = "java(filaDto.getCoordenadas() != null"
            + " && filaDto.getCoordenadas().getLatitud() != null"
            + " ? BigDecimal.valueOf(filaDto.getCoordenadas().getLatitud()) : null)")
    @Mapping(target = "longitud", expression = "java(filaDto.getCoordenadas() != null"
            + " && filaDto.getCoordenadas().getLongitud() != null"
            + " ? BigDecimal.valueOf(filaDto.getCoordenadas().getLongitud()) : null)")
